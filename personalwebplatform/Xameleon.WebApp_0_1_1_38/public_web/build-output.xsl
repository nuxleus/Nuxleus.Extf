...
<xsl:stylesheet version="1.0"
  ...
  exclude-result-prefixes="my html page bVend">
	...
	<xsl:template match="app:collection" mode="workspace">
		<h3>
			<xsl:value-of select="atom:title" /> Collection
		</h3>
		<ul>
			<li>
				Collection Location: <a href="{@href}">
					<xsl:value-of select="@href" />
				</a>
			</li>
			<li>
	...
	<xsl:template match="atom:feed" mode="collection-summary">
		<h4>
			<xsl:value-of select="atom:title" /> Feed Info
		</h4>
		<ul>
			...
			<xsl:if test="atom:entry">
				<li>
					<h4>
						<xsl:value-of select="atom:title" /> Entries
					</h4>
