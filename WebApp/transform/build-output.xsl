...
-->
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:my="http://xameleon.org/service/session" xmlns="http://www.w3.org/1999/xhtml" xmlns:html="http://www.w3.org/1999/xhtml" xmlns:bVend="http://xameleon.org/service/browservendors" xmlns:page="http://xameleon.org/service/page/output" xmlns:app="http://purl.org/atom/app#" xmlns:atom="http://www.w3.org/2005/Atom" version="1.0" exclude-result-prefixes="my html page bVend">
  <xsl:variable name="vendor" select="system-property('xsl:vendor')"/>
  <xsl:output doctype-system="/resources/dtd/xhtml1-strict.dtd" doctype-public="-//W3C//DTD XHTML 1.0 Strict//EN" cdata-section-elements="script" method="xml" omit-xml-declaration="yes"/>
  ...
  <xsl:template match="atom:title" mode="workspace">
    <h1>
			Summary of Collections in the "<xsl:value-of select="."/>" Workspace
		</h1>
  </xsl:template>
  <xsl:template match="app:collection" mode="workspace">
    <h3><xsl:value-of select="atom:title"/> Collection
		</h3>
    <ul>
      <li>
				Collection Location: <a href="{@href}"><xsl:value-of select="@href"/></a>
			</li>
      <li>
				Accepts: <xsl:value-of select="app:accept"/>
			</li>
      <li>
  ...
  <xsl:template match="atom:feed" mode="collection-summary">
    <h4><xsl:value-of select="atom:title"/> Feed Info
		</h4>
    <ul>
      <xsl:apply-templates select="*[not(atom:title)]" mode="collection"/>
      <xsl:if test="atom:entry">
        <li>
          <h4><xsl:value-of select="atom:title"/> Entries
					</h4>
  ...
  <xsl:template match="atom:updated" mode="collection">
    <li>
			Last Update: <xsl:value-of select="."/>
		</li>
  </xsl:template>
  <xsl:template match="atom:summary" mode="collection">
    <li>
			Entry Summary: <xsl:value-of select="."/>
		</li>
  </xsl:template>
  <xsl:template match="atom:updated" mode="collection">
    <li>
  ...
  <xsl:template match="atom:published" mode="collection">
    <li>
			Date Published: <xsl:value-of select="."/>
		</li>
  </xsl:template>
  <xsl:template match="app:edited" mode="collection">
    <li>
			Date Last Edited: <xsl:value-of select="."/>
		</li>
  </xsl:template>
</xsl:stylesheet>
